...
app = typer.Typer(
    name="nanoiqqi",
    help=f"{__logo__} nanoiqqi - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".nanoiqqi" / "history" / "cli_history"
...
def _make_provider(config: Config):
    ...
    from nanoiqqi.providers.custom_provider import CustomProvider
    from nanoiqqi.providers.routing import RoutingProvider
    ...
    if provider_name == "openai_codex" or model.startswith("openai-codex/"):
        ...
        from nanoiqqi.providers.registry import find_by_name
        spec = find_by_name(provider_name)
        if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
            console.print("[red]Error: No API key configured.[/red]")
            console.print("Set one in ~/.nanoiqqi/config.json under providers section")
            raise typer.Exit(1)
    ...
    if aa_api_key:
        from nanoiqqi.providers.smart_router import SmartRouter
        from nanoiqqi.providers.router_metrics import RouterMetrics
        from nanoiqqi.providers.artificial_analysis import fetch_models, fetch_top_agentic_models
        metrics_path = config.workspace_path / ".nanoiqqi" / "router_metrics.json"
...
def _make_activity_sink(config: Config):
    ...
    from nanoiqqi.agent.activity_sink import make_activity_sink
...
def _make_brain_office_bridge(bus, config, agent_loop):
    ...
    from nanoiqqi.brain_office.bridge import BrainOfficeBridge
...
def gateway(
    ...
    from nanoiqqi.config.loader import load_config, get_data_dir
    from nanoiqqi.bus.queue import MessageBus
    from nanoiqqi.agent.loop import AgentLoop
    from nanoiqqi.channels.manager import ChannelManager
    from nanoiqqi.session.manager import SessionManager
    from nanoiqqi.cron.service import CronService
    from nanoiqqi.cron.types import CronJob
    from nanoiqqi.heartbeat.service import HeartbeatService
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from nanoiqqi.bus.events import OutboundMessage
...
def agent(
    ...
    if logs:
        logger.enable("nanoiqqi")
    else:
        logger.disable("nanoiqqi")
    ...
    if message:
        ...
        from nanoiqqi.bus.events import InboundMessage
...
def channels_status():
    """Show channel status."""
    from nanoiqqi.config.loader import load_config
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".nanoiqqi" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # package bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall nanoiqqi")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from nanoiqqi.config.loader import get_data_dir
    from nanoiqqi.cron.service import CronService
...
def cron_add(
    ...
    from nanoiqqi.config.loader import get_data_dir
    from nanoiqqi.cron.service import CronService
    from nanoiqqi.cron.types import CronSchedule
...
def status():
    ...
    if config_path.exists():
        from nanoiqqi.providers.registry import PROVIDERS
