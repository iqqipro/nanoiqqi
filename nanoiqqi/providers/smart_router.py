...
from nanoiqqi.providers.model_capabilities import (
    ModelCapability,
...
from nanoiqqi.providers.router_metrics import RouterMetrics, DEFAULT_LATENCY_MS
...
def _get_weights(
    ...
    from nanoiqqi.config.schema import DEFAULT_ROUTING_WEIGHTS
...
class SmartRouter(LLMProvider):
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        if routing_profile == "subagent":
            subagent_complexity: str | None = kwargs.pop("subagent_complexity", None)
            subagent_domain: str | None = kwargs.pop("subagent_domain", None)
            return await self._chat_subagent(
                messages,
                tools,
                model,
                max_tokens,
                temperature,
                subagent_complexity=subagent_complexity,
                subagent_domain=subagent_domain,
            )
        if force_model:
    ...
    async def _chat_subagent(
        ...
        max_tokens: int,
        temperature: float,
        subagent_complexity: str | None = None,
        subagent_domain: str | None = None,
    ) -> LLMResponse:
        """
        ...
        # Prefer explicit hints from the main agent when provided and valid; fall back to judge or heuristic.
        hint_complexity = (subagent_complexity or "").strip().lower()
        hint_domain = (subagent_domain or "").strip().lower()
        complexity_label: str | None = None

        if hint_complexity in JUDGE_COMPLEXITY_TO_MIN_CAP:
            complexity_label = hint_complexity
            if hint_domain in JUDGE_DOMAINS:
                domain = hint_domain
            min_capability = JUDGE_COMPLEXITY_TO_MIN_CAP[complexity_label]
            logger.debug(
                "Smart router (subagent) hints: complexity={}, domain={}",
                complexity_label,
                domain,
            )
        elif self._judge_model and task_text:
