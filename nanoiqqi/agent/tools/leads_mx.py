...
# Cantidad por defecto de registros a solicitar a la API INEGI (paginación: registro_final).
# Reduce la carga en la API y el volumen de datos devueltos si el usuario no indica registro_final.
DENUE_DEFAULT_PAGE_SIZE = 25
...
class LeadsMx(Tool):
    ...
    description = (
        ...
        "Si necesitas clave de municipio: consulta skill leads_mx_municipios (read_file nanoiqqi/skills/leads_mx_municipios/SKILL.md). NUNCA inventes claves. "
        ...
        "Paginación: registro_inicial, registro_final (por defecto se piden 25 registros). Entidad 2 dígitos; 00=todo. Distancia máx 5000 m."
    )
    parameters = {
        "type": "object",
        "properties": {
    ...
    async def execute(self, **kwargs: Any) -> str:
        if not self.token:
            return (
                ...
                "Configure tools.leads_mx.token en ~/.nanoiqqi/config.json para usar esta herramienta."
            )
        ...
        if method == "buscar":
            ...
            reg_fin = _int_param(kwargs.get("registro_final"), DENUE_DEFAULT_PAGE_SIZE)
