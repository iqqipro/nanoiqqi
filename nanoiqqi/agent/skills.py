...
class SkillsLoader:
    ...
    def _parse_skill_metadata(self, raw: str) -> dict:
        """Parse skill metadata JSON from frontmatter (supports nanoiqqi and openclaw keys)."""
        try:
            data = json.loads(raw)
            if not isinstance(data, dict):
                return {}
            return data.get("nanoiqqi", data.get("nanobot", data.get("openclaw", {})))
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """Get nanoiqqi metadata for a skill (cached in frontmatter)."""
