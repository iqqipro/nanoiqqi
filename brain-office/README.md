...
A small web app that shows your nanoiqqi agent as a character in a pixel-art office. The character state updates in real time based on agent activity (reading files, typing, running commands, waiting for input).

Inspired by [pixel-agents](https://github.com/pablodelucca/pixel-agents); integrated with this repo’s agent (nanoiqqi). **Brain Office is always on**: the agent always sends activity events; you only configure the backend URL if it’s not the default.

## Architecture

- **nanoiqqi** emits activity events (tool_start, tool_end, waiting_input, turn_end, subagent_start/end) to Brain Office (always enabled).
...
Events are JSON objects. nanoiqqi sends them to `{BRAIN_OFFICE_URL}/events` (default `http://localhost:8765/events`).
...
The agent **always** sends activity to Brain Office. Default URL is `http://localhost:8765`. To override (e.g. when running in Docker), set in `~/.nanoiqqi/config.json`:
...
When the agent runs in Docker with `docker compose run --rm nanoiqqi-cli agent`, the URL is already set in `docker-compose.yml`; you don't need to add this to config. If you run the agent in a different way (e.g. custom container), set the URL to `http://brain-office:8765` or use `NANOIQQI_BRAIN_OFFICE__URL=http://brain-office:8765`.

Then run the agent (e.g. `nanoiqqi agent`). Activity will be sent to the backend and the Brain Office UI will update in real time.

## Docker (all-in-one)

From the repo root you can run Brain Office (backend + frontend) and nanoiqqi in Docker:
...
When you run the agent with the same Compose stack, the URL is already set in `docker-compose.yml` (`NANOIQQI_BRAIN_OFFICE__URL=http://brain-office:8765`), so you don't need to add anything to `~/.nanoiqqi/config.json`. Just run:

```bash
docker compose run --rm nanoiqqi-cli agent
```
